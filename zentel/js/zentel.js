  ...
  return new p5(function(sketch) {
    ...
    sketch.setup = function() {
      ...
      if (!document.getElementById('game-canvas').querySelector('canvas')) {
        console.error("Canvas non attaché à #game-canvas (desktop)");
      }

      // Test de dessin simple
      sketch.background('#0a0a1e');
      sketch.fill('#ff0000');
      sketch.noStroke();
      sketch.rect(10, 10, 50, 50); // Rectangle rouge en haut à gauche
    ...
    function spawnWave() {
      ...
      if (wave < 5) {
        let enemyTypeIndex = sketch.min(wave - 1, ENEMY_TYPES.length - 1);
        enemyTypesToSpawn.push({ type: ENEMY_TYPES[enemyTypeIndex], isBoss: false, isTriangle: false });
      } else if (wave < 10) {
        let normalEnemyIndex = sketch.min(wave - 1, ENEMY_TYPES.length - 1);
      ...
      for (let enemyTypeData of enemyTypesToSpawn) {
        ...
        for (let i = 0; i < enemiesPerType; i++) {
          let startY = sketch.floor(sketch.random(GRID_HEIGHT));
    ...
    sketch.draw = function() {
      if (gameState === 'paused') {
        console.log("Jeu en pause, draw() non exécuté (desktop)");
        return;
      }
      console.log("draw() exécuté, dessin de la grille... (desktop)");
      sketch.background('#0a0a1e');
      drawGrid();
      drawModules();
      drawEnemies();
    ...
    function drawGrid() {
      sketch.stroke('#4a4a6a');
      sketch.strokeWeight(2);
      for (let x = 0; x < GRID_WIDTH; x++) {
        for (let y = 0; y < GRID_HEIGHT; y++) {
          if (x < 2) {
            sketch.fill(ENEMY_ZONE_COLOR);
            sketch.stroke(ENEMY_ZONE_COLOR);
          } else {
            sketch.fill(grid[x][y] === 'base' ? '#4682b4' : '#1a1a2e');
            sketch.stroke('#4a4a6a');
          }
          sketch.rect(x * CELL_SIZE, y * CELL_SIZE, CELL_SIZE, CELL_SIZE);
        }
      }
    }
    ...
    function drawModules() {
      for (let module of modules) {
        ...
        if (module.type === 'wall') {
          sketch.fill(TURRET_TYPES.wall.color);
          sketch.noStroke();
          sketch.rect(module.x * CELL_SIZE, module.y * CELL_SIZE, CELL_SIZE, CELL_SIZE);
          sketch.fill(255);
          sketch.text(symbol, module.x * CELL_SIZE + CELL_SIZE / 2, module.y * CELL_SIZE + CELL_SIZE / 2);
        } else {
          sketch.fill(color);
          sketch.noStroke();
          sketch.rect(module.x * CELL_SIZE, module.y * CELL_SIZE, width, height);
          sketch.fill(255);
          sketch.text(symbol, module.x * CELL_SIZE + width / 2, module.y * CELL_SIZE + height / 2);

          if (module.level > 1) {
            sketch.fill('#ffffff');
            sketch.textSize(10);
            sketch.text(module.level, module.x * CELL_SIZE + width / 2, module.y * CELL_SIZE + height / 2 + 15);
            sketch.textSize(14);
          }

          if (sketch.frameCount % (TURRET_TYPES[module.type].attackRate) === 0) {
            for (let i = 0; i < enemies.length; i++) {
              let enemy = enemies[i];
              if (enemy.isBoss && module.type !== 'projectile') continue;

              let d = sketch.dist(module.x * CELL_SIZE + CELL_SIZE / 2, module.y * CELL_SIZE + CELL_SIZE / 2, enemy.x, enemy.y);
    ...
    function drawEnemies() {
      for (let enemy of enemies) {
        if (enemy.isTriangle) {
          sketch.fill(TRIANGLE_COLOR);
          sketch.noStroke();
          sketch.push();
          sketch.translate(enemy.x, enemy.y);
          sketch.triangle(-10, 10, 0, -10, 10, 10);
          sketch.pop();
        } else {
          sketch.fill(enemy.isBoss ? BOSS_COLOR : ENEMY_COLOR);
          sketch.noStroke();
          sketch.ellipse(enemy.x, enemy.y, 20);
        }

        let hpRatio = enemy.hp / enemy.maxHp;
        let healthColor = hpRatio > 0.6 ? '#55ff55' : hpRatio > 0.3 ? '#ffff55' : '#ff5555';
        sketch.stroke(healthColor);
        sketch.strokeWeight(2);
        sketch.noFill();
        let angle = sketch.map(hpRatio, 0, 1, 0, sketch.TWO_PI);
        sketch.arc(enemy.x, enemy.y, 16, 16, -sketch.PI/2, angle - sketch.PI/2);

        if (enemy.isTriangle && sketch.frameCount % enemy.attackRate === 0) {
          let closestModule = null;
          let closestDist = Infinity;
          for (let module of modules) {
            let d = sketch.dist(enemy.x, enemy.y, module.x * CELL_SIZE + CELL_SIZE / 2, module.y * CELL_SIZE + CELL_SIZE / 2);
            if (d < closestDist) {
              closestDist = d;
        ...
        if (enemy.path.length > 0) {
          ...
          let distToTarget = sketch.dist(enemy.x, enemy.y, targetX, targetY);

          let nextGridX = sketch.floor(targetX / CELL_SIZE);
          let nextGridY = sketch.floor(targetY / CELL_SIZE);
          let canMove = true;
          ...
          if (distToTarget < 5) {
            ...
            if (enemy.pathIndex >= enemy.path.length) {
              ...
              let startX = sketch.floor(enemy.x / CELL_SIZE);
              let startY = sketch.floor(enemy.y / CELL_SIZE);
    ...
    function drawProjectiles() {
      for (let i = projectiles.length - 1; i >= 0; i--) {
        let p = projectiles[i];
        sketch.fill(TURRET_COLOR);
        sketch.ellipse(p.x, p.y, 10);
        let angle = sketch.atan2(p.target.y - p.y, p.target.x - p.x);
        p.x += p.speed * sketch.cos(angle);
        p.y += p.speed * sketch.sin(angle);
        let d = sketch.dist(p.x, p.y, p.target.x, p.target.y);
        if (d < 10) {
    ...
    function drawEnemyProjectiles() {
      for (let i = enemyProjectiles.length - 1; i >= 0; i--) {
        ...
        sketch.fill(TRIANGLE_COLOR);
        sketch.ellipse(p.x, p.y, 8);
        let angle = sketch.atan2(p.targetY - p.y, p.targetX - p.x);
        p.x += p.speed * sketch.cos(angle);
        p.y += p.speed * sketch.sin(angle);
        let d = sketch.dist(p.x, p.y, p.targetX, p.targetY);
        if (d < 5) {
          let targetGridX = sketch.floor(p.targetX / CELL_SIZE);
          let targetGridY = sketch.floor(p.targetY / CELL_SIZE);
    ...
    sketch.mousePressed = function() {
      ...
      let gridX = sketch.floor(sketch.mouseX / CELL_SIZE);
      let gridY = sketch.floor(sketch.mouseY / CELL_SIZE);
      ...
      if (gridX >= 0 && gridX < GRID_WIDTH - 1 && gridY >= 0 && gridY < GRID_HEIGHT) {
        ...
        if (selectedModule && !isDeleteModeActive && !isEvolveModeActive && !grid[gridX][gridY] && energy >= TURRET_TYPES[selectedModule].cost) {
          ...
          let enemyOnCell = enemies.find(enemy => {
            let enemyGridX = sketch.floor(enemy.x / CELL_SIZE);
            let enemyGridY = sketch.floor(enemy.y / CELL_SIZE);
    ...
    document.getElementById('heal-base').addEventListener('click', () => {
      if (wave >= 5 && xp >= 500 && base.hp < BASE_HP) {
        xp -= 500;
        base.hp = sketch.min(BASE_HP, base.hp + 10);
        updateStats();
      }
    });
    ...
    sketch.windowResized = function() {
      let scaleFactor = sketch.min(sketch.windowWidth / canvasWidth, sketch.windowHeight / canvasHeight);
      sketch.resizeCanvas(canvasWidth * scaleFactor, canvasHeight * scaleFactor);
    };
    ...
    function calculateCellCost(x, y) {
      ...
      for (let module of modules) {
        ...
        let distance = sketch.dist(x * CELL_SIZE + CELL_SIZE / 2, y * CELL_SIZE + CELL_SIZE / 2, module.x * CELL_SIZE + CELL_SIZE / 2, module.y * CELL_SIZE + CELL_SIZE / 2);
